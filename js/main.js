...
const createParrotBox = (product) => {
  ...
  elParrotFeatures.textContent = features.split();

  // elParrotFeatures.forEach( list=> {
...
const renderParrots = (parrotsArray = showingParrots) => {
  ...
  parrotsArray.forEach((product1) => {
    ...
    elParrotWrapper.append(elParrotBox);
  });
};
renderParrots();
...
elAddParrotForm.addEventListener("submit", (e) => {
  ...
  if (formTitle && formPrice && formBrithDay && formWidth && formHeight > 0) {
    ...
    showingParrots.unshift(addingProduct);
...
//?  Edit va Delete ishlatish uchun olib kelingan ozgaruvchilar
...
elParrotWrapper.addEventListener("click", (evt) => {
  if (evt.target.matches(".btn-trash")) {
    ...
    const clickedShowingBtnIndex = products.findIndex(
      (product) => product.id === clickedBtn
    );

    products.splice(clickedBtnIndex, 1);

    showingParrots.splice(clickedShowingBtnIndex, 1);
...
elEditForm.addEventListener("submit", (e) => {
  ...
  if (
    ...
    const editingItemIndex = products.findIndex(
      (product) => product.id === editingId
    );

    const editingShowingItemIndex = showingParrots.findIndex(
      (product) => product.id === editingId
    );
    ...
    showingParrots.splice(editingShowingItemIndex, 1, editProduct);
